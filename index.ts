...
async function fetchModFileInfo(projectId: string | number, fileId: string | number): Promise<ModFileInfo> {
...
async function downloadMods(modpack: JSZip, targetDir: string) {
    ...
    const manifest = JSON.parse(await manifestData.async('text')) as ModpackManifest;
